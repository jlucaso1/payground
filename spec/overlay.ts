...
export const OVERLAY: readonly OverlayEntry[] = [
  {
    schema: 'Payment',
    note: '`point_of_interaction` and `barcode` are absent from spec3.json, so a response carries no Pix QR code and no boleto barcode. The shapes come from the Pix guide sample and from the sdk-nodejs PointOfInteraction type.',
    source:
  ...
  {
    schema: 'Refund',
    note: 'The spec enumerates `approved`, `in_process` and `rejected`; the API also reports `cancelled`. It has no `reason`, no `unique_sequence_number`, no `amount_refunded_to_payer`, no `adjustment_amount` and no `metadata`, and it closes `refund_mode` to standard or instant where the SDK declares it nullable.',
  ...
  },
  {
    ...
    note: 'The spec requires only `transaction_amount` and `payer`, and has no `three_ds_mode` for cards and no `point_of_interaction` to name the channel a Pix payment came from. `payment_method_id` is restated exactly as the spec types it, because payground rejects a payment without it unless a card token supplies the brand.',
  ...
  {
    schema: 'POS',
    note: 'The same `allOf` gap as `Store`. The flattened shape also carries what the spec omits: the numeric `id` the API returns, the owning `user_id`, `status`, and the `qr` object holding the image and the printable templates that a point of sale exists to hand out.',
  ...
  {
    schema: 'ReportConfig',
    note: 'The spec names the delimiter `separator` and has no scheduling flag. The account report endpoints read `column_separator`, which also accepts a tab, and report whether the schedule is enabled.',
    source:
  ...
  {
    schema: 'ReportTask',
    note: 'The task response carries the name of the generated file. spec3.json omits it, and without it a client cannot build the download URL.',
  ...
  {
    schema: 'ReportEntry',
    note: 'GET /v1/account/release_report/list reports the schedule that produces each file. spec3.json reuses the plain report entry and drops `frequency` and `enabled`.',
  ...
  {
    schema: 'ReportTask',
    note: 'The settlement task also names the file it produces, and that name is the path segment the download endpoint takes. The spec exposes only the finished download URL.',
    source:
  ...
  {
    schema: 'Claim',
    note: 'The spec omits the payment the claim was opened against, the parent claim, the site, the status history that `/status_history` returns, and the resolution recorded when a dispute ends.',
  ...
  {
    schema: 'ClaimMessage',
    note: 'The message list carries the claim it belongs to, the flat `sender_role` and `receiver_role` pair the notification payload uses, and the stage the message was written in. The spec models only the nested `from` object.',
  ...
  },
  {
    ...
    note: 'The attachment responses key a file by `file_id`, which `ClaimEvidence` lacks. Shipping evidence can also be a plain `value`, a tracking code rather than a file, so the file fields are nullable. The request enum admits `proof_of_delivery`, which the resource enum drops.',
  ...
  {
    schema: 'ClaimReason',
    note: 'A reason carries its long `detail` text, the group it belongs to and the flow that produced it. The spec keeps only `id`, `description` and `type`.',
  ...
  },
  {
    ...
    note: 'Each expected resolution has an `id` and a currency, and says which side it favours. The spec omits all three, so two options of the same type are indistinguishable.',
